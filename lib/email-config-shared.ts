...
/** Per-contact quick replies layered on top of the global list */
export type ContactReplySet = {
  contactEmail: string;
  contactName?: string;
...
export type EmailConfig = {
  ...
  contactReplies: ContactReplySet[];
};
...
export type WritingProfile = {
  ...
  updatedAt: string;
  sampleCount: number;
  /** Suggested tone rules awaiting user accept/reject */
...
export function buildReplyPromptContext(config: EmailConfig, intent: string): string {
  ...
  const template =
    ...
    config.contactReplies.flatMap((c) => c.replies).find((r) => r.id === intent) ??
...
/** Merge global replies with contact-specific ones (contact first). */
export function repliesForContact(
  config: EmailConfig,
  contactEmail?: string | null
): QuickReplyTemplate[] {
  if (!contactEmail) return config.replies;
  const email = contactEmail.toLowerCase();
  const match = config.contactReplies.find((c) => c.contactEmail.toLowerCase() === email);
  if (!match?.replies.length) return config.replies;
  const customIds = new Set(match.replies.map((r) => r.id));
  return [...match.replies, ...config.replies.filter((r) => !customIds.has(r.id))];
}
