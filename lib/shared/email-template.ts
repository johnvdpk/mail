import { DEFAULT_SIGNATURE_TEXT } from "../config/email-config-shared";
...
function signatureHtmlFor(formatting?: MailFormatting): string {
  const fontFamily = formatting?.fontFamily ?? "Arial, sans-serif";
  const signatureText = formatting?.signatureText ?? DEFAULT_SIGNATURE_TEXT;
  return signatureText === DEFAULT_SIGNATURE_TEXT
    ? SIGNATURE_HTML
    : buildPlainSignatureHtml(signatureText, fontFamily);
}

/** Insert the personal signature into a pre-built HTML body. */
export function appendSignatureToHtml(html: string, formatting?: MailFormatting): string {
  const signatureHtml = signatureHtmlFor(formatting);
  if (/<\/body>/i.test(html)) {
    return html.replace(/<\/body>/i, `${signatureHtml}\n</body>`);
  }
  return `${html}\n${signatureHtml}`;
}
