...
import type { CalendarInvite } from "../calendar/ics";
...
export async function writeFolderCache(cache: FolderCache): Promise<void> {
  await transaction(async (client) => {
    ...
    for (const m of cache.messages) {
      // Conflict target must be (folder, uid), not id: concurrent syncs of the
      // same folder (e.g. an overlapping page load + client sync) can race on
      // this exact row, and only the constraint that's actually hit resolves
      // the conflict cleanly instead of raising a raw unique-violation error.
      await client.query(
...
async function refreshEmbeddingForBody(messageId: string, textBody: string): Promise<void> {
  const { isOpenRouterConfigured } = await import("../ai/openrouter");
