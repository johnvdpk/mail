...
type FinalizeResponse = { title?: string; description?: string };
...
export function TicketChat({ onCreate, submitting, onCancel }: Props) {
  ...
  async function onStartTicket() {
    ...
    setError(null);
    try {
      const data = await apiRequest<FinalizeResponse>("/api/tickets/draft-chat/finalize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages }),
      });
      if (!data.title || !data.description) {
        setError("Ticket samenstellen mislukt");
        return;
      }
      setDraft({ title: data.title, description: data.description });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Ticket samenstellen mislukt");
    } finally {
      setFinalizing(false);
    }
  }
